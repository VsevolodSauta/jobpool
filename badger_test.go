...
func TestBadgerBackend_EnqueueDequeue(t *testing.T) {
	...
	backend, err := jobpool.NewBadgerBackend(tmpDir)
	if err != nil {
		t.Fatalf("Failed to create BadgerDB backend: %v", err)
	}
	defer func() {
		_ = backend.Close()
	}()
...
func TestBadgerBackend_AssigneeTracking(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "test_jobpool_*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
...
func TestBadgerBackend_JobStats(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "test_jobpool_*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
